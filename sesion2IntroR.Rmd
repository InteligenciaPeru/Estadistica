...
Toda la información ha sido '_transferida_' a **datosLeidos**. El archivo en la computadora estaba en el folder _data_, y se llama _tips.csv_. Un archivo de este tipo es similar a un archivo en Excel, de hecho puedes abrirlo en Excel facilmente.
...
Usemos la versatil función **lapply**, que aplica una función a las columnas. La función a aplicar es **as.factor**, que puede convertir numeros o texto en categorias o _factores_ en el jerga del R:

```{r}
datosLeidos[,c(4:7)]=lapply(datosLeidos[,c(4:7)],as.factor)
```

Cuando escribimos _datosLeidos[,c(4:7)]_ hemos llamada a un subconjunto de los datos. Esto se debe entender así. Las variables que queriamos transformas están en las columnas de la _4_ a la _7_ del objeto _datosLeidos_. Si uno escribe _datosLeidos[1,1]_ R devuelve una celda (primera fila, primera columna). Si escribimos _datosLeidos[,4)]_ devuelve todas las filas de la columna 4. 
...
Eliminemos columnas de los rankings. Primero veamos que columnas hay:
```{r, eval=FALSE}
names(provincias)
```
X1 y X3 son necesarias. Eliminemos todas las otras que comiencen con X:
...
Si aplicas una función numerica, y sale error, es por que quizas los datos tengan problemas:
```{r, eval=FALSE}
str(provincias)
```

Las funciones numericas no pueden aplicarse a los textos ('chr'), por ende:
```{r, eval=FALSE}
